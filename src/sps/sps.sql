...
AS
BEGIN
    SELECT j.*, n.pais AS nacionalidad, c.nombre_club, c.liga
    FROM Jugador j
...
CREATE PROCEDURE sp_top_jugadores_por_habilidad
...
CREATE PROCEDURE sp_actualizar_rating
    @id_evaluacion INT,
...
CREATE PROCEDURE sp_cambiar_club_jugador
    @id_jugador INT,
    @nuevo_id_club INT,
    @fecha_inicio DATE
AS
...
CREATE PROCEDURE sp_top5_semanal_por_posicion
    @fecha_inicio DATE,
    @fecha_fin DATE
AS
BEGIN
